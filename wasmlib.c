...
extern unsigned long js_compile_to_bytecode(char * source, unsigned long * buf_len);

int EMSCRIPTEN_KEEPALIVE eval_js_source(char * source) {
    return js_eval(source);
}

unsigned long EMSCRIPTEN_KEEPALIVE compile_to_bytecode(char * source, unsigned long * buf_len) {
    return js_compile_to_bytecode(source, buf_len);
}
